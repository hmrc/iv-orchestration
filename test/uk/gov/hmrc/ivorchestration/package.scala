...
package object ivorchestration {
  ...
  val sampleAuthRetrieval = AuthRetrieval("777", Some("123455"),200,
    Some(DateTime.now),Some("123"),Some(sampleItmpAddress),Some("BN13 3AS"),Some("Matt"),Some("Groom"), Some(LocalDate.now))

  val sampleAuthRetrievalCore = AuthRetrievalCore(sampleAuthRetrieval, JourneyId("123"), DateTime.now(DateTimeZone.UTC))

  val buildRetrievalCore: AuthRetrieval => AuthRetrievalCore = retrieval => AuthRetrievalCore(retrieval, JourneyId("123"), new DateTime)
